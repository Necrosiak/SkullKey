...
def _ssl_context():
    """plugin_loader's embedded Python (PyInstaller) has no CA bundle →
    urllib fails with CERTIFICATE_VERIFY_FAILED. Point to the system bundle."""
...
def _replace_file(src: Path, dst: Path) -> None:
    ...
    try:
        ...
        if dst.suffix in (".sh", ".py"):
            # chmod on the tmp (ours) — chmod-ing a root-owned dst was the
