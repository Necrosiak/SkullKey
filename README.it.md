...
- 🐧 **Compatibilità** — lavoriamo attivamente per supportare ogni OS in grado di eseguire Steam in modalità gioco / Big Picture (Linux per ora): rilevamento portabile, nessuna assunzione specifica di distribuzione Note per distribuzione: [docs/OS-NOTES.md](docs/OS-NOTES.md).

## Installazione

Via Decky Loader, senza passare dal desktop:

1. Installate [Decky Loader](https://decky.xyz/)
2. Attivate la **modalità sviluppatore** nelle impostazioni generali di Decky
3. Impostazioni Decky → **Sviluppatore** → *Installa plugin da URL*:
   `https://github.com/Necrosiak/SkullKey/releases/latest/download/SkullKey.zip`

Oppure compilate dai sorgenti:
...
Un bug, qualcosa che si inceppa, un comportamento strano sulla vostra
distribuzione? Un'idea? **Aprite una
[issue](https://github.com/Necrosiak/SkullKey/issues)** — ogni segnalazione
orienta direttamente ciò che verrà costruito dopo, e nessuna segnalazione è
troppo piccola. Per aiutarci a correggere in fretta, includete se possibile:

- distribuzione e versione (Bazzite 42, CachyOS, Ubuntu 24.04…) e come gira Steam (modalità gioco / Big Picture / desktop)
- la versione del plugin (Impostazioni → Informazioni) e lo store/scheda coinvolto
- cosa avete fatto, cosa vi aspettavate, cosa è successo invece
- i log: `~/homebrew/logs/SkullKey/` (problemi di dipendenze: `ensure_deps.log`)

Le richieste di funzionalità e i «funziona!» su configurazioni insolite
valgono altrettanto — ci dicono cosa supportare dopo.
