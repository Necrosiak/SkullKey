...
// Steam's internal collection store (not typed by decky-frontend-lib)
...
export const GameDetailsItem: VFC<GameDetailsItemProperties> = ({ serverAPI, shortname, initActionSet, closeModal }) => {
    ...
    // Files the shortcut into a Steam collection named after the store
    // ("Epic", "GOG", ...) so installed games stay grouped in the library.
    const addToStoreCollection = async (appId: number) => {
        try {
            const storeName = initActionSet.replace(/Actions$/, "");
            if (!storeName || typeof collectionStore === "undefined") {
                return;
            }
            let collection = collectionStore.userCollections.find(
                (c: any) => c.displayName === storeName);
            if (!collection) {
                collection = collectionStore.NewUnsavedCollection(storeName, undefined, []);
                await collection.Save();
            }
            const overview = appStore.GetAppOverviewByAppID(appId);
            if (overview && collection.AsDragDropCollection) {
                collection.AsDragDropCollection().AddApps([overview]);
                await collection.Save();
            }
        } catch (error) {
            logger.error("addToStoreCollection", error);
        }
    };
