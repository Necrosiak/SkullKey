...
import { ActionCard, IconBtn, storeTheme } from './Styled';
import { t } from '../i18n';
...
const GameDisplay: VFC<GameDisplayProperties> = (
    ...
    const contextMenu = (e: any) => {
        showContextMenu(
            <Menu label={t("menu_config")} cancelText={t("menu_cancel")} onCancel={() => { }}>
                {editors.map((editor) => {
    ...
    const actionsMenu = (e: any) => {
        showContextMenu(
            <Menu label={t("menu_actions")} cancelText={t("menu_cancel")} onCancel={() => { }}>
                ...
                {isInstalled &&
                    <>
                        <MenuItem onSelected={resetLaunchOptions}>{t("reset_launch_options")}</MenuItem>

                    </>
                }
                <MenuItem onSelected={() => showModal(<ConfirmModal strTitle={t("confirm")} strDescription={t("uninstall_q", { name })} onOK={() => { uninstaller(); }} />)}> {/*pass uninstall fn like this so it doesn't wait for the async fn to close the modal */}
                    {t("uninstall_game")}
                </MenuItem>
                        ...
                        return <MenuItem onSelected={
                            () => {
                                ...
                                if (action.Type == "ScriptActionConfirm") {
                                    showModal(<ConfirmModal strTitle={t("confirm")} strDescription={action.Title + '?'} onOK={() => { scriptRunner(initActionSet, action.ActionId, args); }} />);
                                }
                                else {
    ...
    const focusableProps: FooterLegendProps = !isInstalled ? {} :
        {
            ...
            onOptionsActionDescription: t('go_steam_page')
        };
            ...
            >
                <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '30px 0' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        {(() => {
                            const StoreIcon = storeTheme(initActionSet).icon;
                            return <StoreIcon size={20} style={{ color: storeTheme(initActionSet).color, flexShrink: 0 }} />;
                        })()}
                    ...
                    <div>
                        {installing ? (
                            <div key={'download'} style={{ margin: '5px 2px 10px', animation: 'fadeIn .3s ease-in-out forwards' }}>
                                <div style={{ marginBottom: '5px', color: storeTheme(initActionSet).color, fontSize: '11px', lineHeight: '13px', whiteSpace: 'pre-wrap' }}>
                        ...
                        <Focusable style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                            <div style={{ flex: 1 }}>
                                <ActionCard
                                    big
                                    active
                                    color={installing ? '#f44336' : isInstalled ? '#4caf50' : storeTheme(initActionSet).color}
                                    onClick={installing ? cancelInstall : !isInstalled ? installer : runner}
                                >
                                    {installing ? <FaTimes size={14} /> : !isInstalled ? <FaDownload size={14} /> : <FaPlay size={14} />}
                                    <span>{installing ? t('btn_cancel') : !isInstalled ? t('btn_install_game') : t('btn_play')}</span>
                                </ActionCard>
                            </div>
                            <IconBtn color={storeTheme(initActionSet).color} onClick={actionsMenu}>
                                <FaSlidersH style={{ verticalAlign: 'middle' }} />
                            </IconBtn>
                            {editors.length > 0 && (
                                <IconBtn color={storeTheme(initActionSet).color} onClick={contextMenu}>
