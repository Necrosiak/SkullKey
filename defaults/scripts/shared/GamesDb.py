...
class GamesDb(GameSet.GameSet):
    ...
    def get_base64_images(self, game_id, image_prefix="", url_encode=False):
        ...
        c.execute("SELECT Title FROM Game WHERE ShortName=?", (game_id,))
        row = c.fetchone()
        ...
        for row in c.fetchall():
            print(f"{row['Type']}: {row['ImagePath']}", file=sys.stderr)
            by_type.setdefault(row['Type'], row['ImagePath'])
        conn.close()
        ...
        # Steam artwork slots: Grid = vertical capsule, GridH = wide capsule
        # (home screen / recents), Hero = library page header, Logo = overlay.
        # Candidates per slot, best first: Steam's official artwork (looks
        # native in the library), then gamesdb art with fallbacks so shortcuts
        # never end up with an empty capsule when data is partial.
        candidates = {
            'Grid': [pick('vertical_cover', 'cover', 'square_icon')],
            'GridH': [pick('horizontal_artwork', 'background', 'cover', 'artwork', 'screenshot', 'vertical_cover')],
            'Hero': [pick('background', 'horizontal_artwork', 'artwork', 'screenshot', 'vertical_cover')],
            'Logo': [pick('logo')],
        }
        appid = self.get_steam_appid(title) if title else None
        if appid:
            print(f"steam artwork found: {title} -> appid {appid}", file=sys.stderr)
            base = f"{self.STEAM_CDN}/{appid}"
            candidates['Grid'].insert(0, f"{base}/library_600x900_2x.jpg")
            candidates['GridH'].insert(0, f"{base}/header.jpg")
            candidates['Hero'].insert(0, f"{base}/library_hero.jpg")
            candidates['Logo'].insert(0, f"{base}/logo.png")
        downloaded = {}
        content = {}
        for slot, urls in candidates.items():
            content[slot] = None
            for url in urls:
                if not url:
                    continue
                if url in downloaded:
                    content[slot] = downloaded[url]
                    break
                try:
        ...
        return json.dumps({'Type': 'Images', 'Content': content})
